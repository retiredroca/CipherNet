...
(function() {
  ...
  async function importKey() {
    ...
    if (isPQImport) {
      ...
      const probe    = 'ciphernet-import-check:' + fp + ':' + Date.now();
      const probeSig = await Crypto.signDataPQ(probe, secretKey);
      const bound    = await Crypto.verifyDataPQ(probe, probeSig, stored0.publicKeyPem);
      if (!bound) {
        showLoginError('This PQ private key does not match the public key stored for this browser identity. ' +
          'Paste the correct private key, or restore a full backup first.');
        btn.textContent = 'IMPORT AND ENTER'; btn.disabled = false; return;
      }
    ...
    const users  = getStoredUsers();
    const sanitizeHandle = window.CipherNet.Util.sanitizeHandle;
    const storedHandle   = users[fp] && users[fp].handle;
    let handle = handleOverride || storedHandle || 'user_' + fp.slice(0,6);
    if (handleOverride && !window.CipherNet.Util.VALID_HANDLE.test(handleOverride)) {
      showLoginError('Handle: 3-32 chars, letters/numbers/underscores only.');
      btn.textContent = 'IMPORT AND ENTER'; btn.disabled = false; return;
    }
    handle = sanitizeHandle(handle, fp);
  ...
  function switchLockTab(tab) {
    ['register', 'login', 'unlock'].forEach(n => {
      ...
      if (t) t.classList.toggle('active', n === tab);
      if (p) p.classList.toggle('hidden',  n !== tab);
    });
  }
  ...
  function onAuthenticated() {
    ...
    $('auth-btn').textContent = '[ ' + state.me.handle.toUpperCase() + (state.me.guest ? ' // GUEST' : '') + ' ]';
    $('identity-actions').classList.remove('hidden');
    $('identity-actions').classList.add('visible');
    const sh = document.querySelector('.storage-health');
    if (sh) sh.style.display = state.me.guest ? 'none' : '';
    if (!state.me.guest) window.CipherNet.Util.showStorageWarning();
