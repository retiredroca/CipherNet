...
(function() {
  ...
  async function enableNostr() {
    ...
    if (ok) {
      subscribeNostrDMs();
      // Channel subscriptions are owned by CipherChannels (C3): every joined
      // channel plus the public discovery stream, via NIP-28 kind 40/42.
      if (window.CipherChannels) window.CipherChannels.subscribeNostr();
      updateNostrStatusBar();
