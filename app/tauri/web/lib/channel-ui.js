...
(function() {
  ...
  function switchChannel(channelId) {
    ...
    // Ensure we're subscribed to this channel's live Nostr stream
    if (window.CipherNostr && window.CipherNostr.isReady() && window.CipherChannels
        && !window.CipherChannels.isSubscribed(channelId) && window.CipherChannels.getJoined().includes(channelId)) {
      window.CipherChannels.subscribeNostr();
    }
  }
  ...
  function buildChannelRow(ch, role, hasKey, joined) {
    ...
    nameEl.className = 'ch-row-name';
    nameEl.textContent = (ch.passphrase ? '\ud83d\udd12 ' : '# ') + ch.name;
    info.appendChild(nameEl);

    const safeType = (ch.type === 'private') ? 'private' : 'public';
    const typeEl = document.createElement('span');
    typeEl.className = 'ch-row-type ' + safeType;
    typeEl.textContent = safeType.toUpperCase();
    info.appendChild(typeEl);

    if (role === 'owner') {
      const r = document.createElement('span'); r.className = 'ch-row-role'; r.textContent = 'OWNER';
      info.appendChild(r);
    }
    if (role === 'admin') {
      const r = document.createElement('span'); r.className = 'ch-row-role'; r.textContent = 'ADMIN';
      info.appendChild(r);
    }
    if (ch.description) {
      const d = document.createElement('div'); d.className = 'ch-row-desc'; d.textContent = ch.description;
      info.appendChild(d);
    }
  ...
  document.addEventListener('DOMContentLoaded', function initChannelUI() {
    ...
    $('ch-settings-archive-btn').addEventListener('click', async () => {
      ...
      try {
        ...
        renderChannelList();
        // Leave the deleted channel if we were viewing it
        if (state.channel === id) {
          const rest = window.CipherChannels.getJoined();
          if (rest.length) switchChannel(rest[0].id);
          else {
            state.channel = null;
      ...
      window._channelNostrHandler = async (channelId, event) => {
        ...
        if (state.view === 'channel' && state.channel === channelId) {
          const aesKey = window.CipherChannels.getActiveKey(channelId)
                      || (ch.passphrase ? null : await window.CipherChannels.getOpenKey(channelId));
          if (aesKey) {
            try {
