...
(function() {
  ...
  // Resolve the AES write/read key for a channel. Open channels use the
  // deterministic public key derived from the channel id; passphrase
  // channels need the user to have entered the passphrase.
  function resolveChannelKey(channel) {
    const ch = window.CipherChannels && window.CipherChannels.get(channel);
    if (ch) {
      if (window.CipherChannels.getActiveKey(channel)) return window.CipherChannels.getActiveKey(channel);
      if (ch.passphrase) return null;
      return window.CipherChannels.getOpenKey(channel);
    }
    return state.channelKeys[channel];
  }

  async function sendChannelMessage(text) {
    const aesKey = await resolveChannelKey(state.channel);
  ...
  async function loadChannelHistory(channel) {
    const aesKey = await resolveChannelKey(channel);
  ...
  function addDMSidebarItem(peer) {
    ...
    div.appendChild(document.createTextNode(peer.handle));
    const fpTag = document.createElement('span');
    fpTag.className = 'user-fp';
    fpTag.textContent = peer.fingerprint.slice(0,6);
    div.appendChild(fpTag);
  ...
  async function setChannelPassphrase() {
    ...
    try {
      if (window.CipherChannels && window.CipherChannels.get(state.channel)) {
        const ok = await window.CipherChannels.activateKey(state.channel, pass);
        if (!ok) throw new Error('Key derivation failed');
      } else {
        // Legacy built-in channel — derive with 600k, fall back to 200k so
        // pre-upgrade history remains readable.
        let stored = [];
        try { stored = JSON.parse(localStorage.getItem('cipher_msgs_' + state.channel) || '[]'); }
        catch { stored = []; }
        const first = (Array.isArray(stored) && stored.length && typeof stored[0].ciphertext === 'string') ? stored[0] : null;
        let legacyKey = first ? await Crypto.deriveChannelKey(pass, state.channel, Crypto.CHANNEL_ITERS_LEGACY) : null;
        let legacyOk  = false;
        if (legacyKey && first) {
          try { await Crypto.aesDecrypt(first.ciphertext, legacyKey); legacyOk = true; } catch { legacyOk = false; }
        }
        state.channelKeys[state.channel] = legacyOk ? legacyKey : await Crypto.deriveChannelKey(pass, state.channel);
      }
