...
(function() {
  ...
  // Derive a local AES wrap key bound to the identity's signing key.
  // The plaintext private keys that used to sit in localStorage are now
  // encrypted with this key, so at-rest data is no longer readable.
  async function deriveIdentityWrapKey(signingKey, algo) {
    let material;
    if (algo && algo.name === 'ML-DSA-65') {
      material = new Uint8Array(signingKey).slice(0, 32);
    } else if (algo && algo.name === 'RSA-PSS') {
      const raw = await crypto.subtle.exportKey('pkcs8', signingKey);
      material = new Uint8Array(await crypto.subtle.digest('SHA-256', raw));
    } else {
      const jwk = await crypto.subtle.exportKey('jwk', signingKey);
      material = padScalar(b64urlToBytes(jwk.d));
    }
    if (!material || material.length === 0) throw new Error('Cannot derive local wrap key from this key type');
    const base = await crypto.subtle.importKey('raw', material, 'HKDF', false, ['deriveKey']);
    const salt = await crypto.subtle.digest('SHA-256', new TextEncoder().encode('ciphernet-local-wrap'));
    return crypto.subtle.deriveKey(
      { name: 'HKDF', hash: 'SHA-256', salt, info: new TextEncoder().encode('ciphernet-local-wrap:v1') },
      base, { name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt']
    );
  }

  async function wrapWithIdentityKey(signingKey, algo, bytes) {
    const k  = await deriveIdentityWrapKey(signingKey, algo);
    const iv = crypto.getRandomValues(new Uint8Array(12));
  ...
  async function unwrapWithIdentityKey(signingKey, algo, b64) {
    const k   = await deriveIdentityWrapKey(signingKey, algo);
  ...
  const PACK_PREFIX = 'CIPHER-PACK:v1:';
  const PACK_ITERS  = 600_000;

  async function encryptIdentityPack(password, bytes) {
    const enc     = new TextEncoder();
    const salt    = crypto.getRandomValues(new Uint8Array(16));
    const iv      = crypto.getRandomValues(new Uint8Array(12));
    const baseKey = await crypto.subtle.importKey('raw', enc.encode(password), 'PBKDF2', false, ['deriveKey']);
    const aesKey  = await crypto.subtle.deriveKey(
      { name: 'PBKDF2', salt, iterations: PACK_ITERS, hash: 'SHA-256' },
      baseKey, { name: 'AES-GCM', length: 256 }, false, ['encrypt']
    );
    const ct = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, aesKey, bytes);
    const out = new Uint8Array(16 + 12 + ct.byteLength);
    out.set(salt, 0); out.set(iv, 16); out.set(new Uint8Array(ct), 28);
    return PACK_PREFIX + bytesToB64(out);
  }

  async function decryptIdentityPack(encStr, password) {
    if (!encStr.startsWith(PACK_PREFIX))
      throw new Error('Not a valid identity pack');
    const enc     = new TextEncoder();
    const buf     = b64ToBytes(encStr.slice(PACK_PREFIX.length));
    const salt    = buf.slice(0, 16);
    const iv      = buf.slice(16, 28);
    const ct      = buf.slice(28);
    const baseKey = await crypto.subtle.importKey('raw', enc.encode(password), 'PBKDF2', false, ['deriveKey']);
  ...
  // Channel key derivation: 600k PBKDF2 iterations (upgraded from 200k).
  // expose both so older history encrypted with the legacy iterations can
  // still be read via fallback detection.
  const CHANNEL_ITERS       = 600000;
  const CHANNEL_ITERS_LEGACY = 200000;

  async function deriveChannelKey(passphrase, channel, iterations) {
    const enc  = new TextEncoder();
    const base = await crypto.subtle.importKey('raw', enc.encode(passphrase), 'PBKDF2', false, ['deriveKey']);
    const salt = await crypto.subtle.digest('SHA-256', enc.encode('cipher-channel:' + channel));
    return crypto.subtle.deriveKey(
      { name: 'PBKDF2', salt, iterations: iterations || CHANNEL_ITERS, hash: 'SHA-256' },
      base, { name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt']
    );
  }
  ...
  window.CipherNet.Crypto = {
    ...
    deriveIdentityWrapKey, wrapWithIdentityKey, unwrapWithIdentityKey,
    ...
    PACK_PREFIX, PACK_ITERS, encryptIdentityPack, decryptIdentityPack,
    deriveChannelKey, CHANNEL_ITERS, CHANNEL_ITERS_LEGACY,
  };
})();
