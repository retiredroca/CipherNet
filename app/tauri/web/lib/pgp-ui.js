    ...
    try {
      ...
      if (sigEl) {
        if (!senderArmored) {
          ...
          // Decrypted fine, but the message was never signed — that is not a
          // signature failure, just an unsigned message.
          sigEl.textContent = '\u26a0 Message NOT SIGNED (encrypted only)';
          sigEl.className   = 'pgp-sig-status warn';
        } else {
          try {
