...
(function() {
  ...
  const WORD_OF = new Map();
  WORDS.forEach((w, i) => WORD_OF.set(w, i));
  ...
  async function generateMnemonic() {
    ...
    for (let i = 0; i < 8; i++) bits.push((hash[0] >> (7 - i)) & 1);
    const out = [];
  ...
  async function validateMnemonic(phrase) {
    try {
      ...
      for (const w of words) {
        if (!WORD_OF.has(w)) return false;
        indices.push(WORD_OF.get(w));
      }
      const bits = [];
      ...
      for (let i = 0; i < 8; i++) {
        if (((hash[0] >> (7 - i)) & 1) !== bits[256 + i]) return false;
      }
      return true;
    } catch { return false; }
  }

  window.CipherNet.Bip39 = { WORDS, generateMnemonic, validateMnemonic };
})();
