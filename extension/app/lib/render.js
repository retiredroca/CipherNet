...
(function() {
  ...
  function updateMsgInput() {
    ...
    if (state.view === 'dm') {
      ...
      const ch      = window.CipherChannels && window.CipherChannels.get(state.channel);
      const openKey = ch && !ch.passphrase;
      const hasKey  = !!state.channelKeys[state.channel]
                   || (window.CipherChannels && window.CipherChannels.canWrite(state.channel));
      enabled     = hasKey;
      placeholder = hasKey
        ? 'Message #' + state.channel + ' (AES-256-GCM encrypted)...'
        : (openKey ? 'Joining #' + state.channel + '...' : 'Set a channel passphrase to send messages...');
      ...
      hint        = state.me
        ? (hasKey
            ...
            : (openKey ? '> JOINING OPEN CHANNEL...' : '> SET A CHANNEL PASSPHRASE TO ENABLE ENCRYPTION'))
        : '';
    }
